...
export default function Step3({ nextStep }: NextStep) {
  ...
  const handleSetTimeIntervals = async (data: any) => {
    try {
      ...
      await api.post('/users/intervalos', { intervals });
  ...
  return (
    <div className="mt-6">
      <form onSubmit={handleSubmit(handleSetTimeIntervals)} className="w-full flex flex-col rounded-lg gap-2 bg-gray-800 p-4">
        ...
        {errors.intervals && (
          <FormError message={errors.intervals?.root?.message} />
        )}

        <Button disabled={isSubmitting} className="text-white bg-ignite-300 text-sm px-2 py-3 font-medium rounded-lg flex gap-2 justify-center items-center">
          <FaArrowRight />
          Próximo passo
        </Button>
      </form>
